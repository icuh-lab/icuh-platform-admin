document.addEventListener('DOMContentLoaded', function() {
    ...
    if (pendingTab) console.log('삭제 대기 중 탭 찾음');
    if (approvedTab) console.log('삭제 완료됨 탭 찾음');
});
...
async function fetchPendingArticles() {
    try {
        ...
        updateTotalCount(articles.length, '삭제 대기 중');
...
async function fetchApprovedArticles() {
    try {
        console.log('삭제 완료됨 게시글 요청 시작');
        const response = await fetch('/api/v1/admin/articles/delete-approved');
        ...
        const articles = await response.json();
        console.log('삭제 완료됨 게시글 수신 완료:', articles.length);
        displayArticles(articles);
        updateTotalCount(articles.length, '삭제 완료됨');
        setupPagination(articles);
    } catch (error) {
        console.error('게시글 데이터를 가져오는 중 오류 발생:', error);
        handleFetchError();
