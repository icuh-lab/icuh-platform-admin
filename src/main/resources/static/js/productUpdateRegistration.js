document.addEventListener('DOMContentLoaded', function() {
    ...
    if (pendingTab) console.log('수정 대기 중 탭 찾음');
    if (approvedTab) console.log('수정 완료됨 탭 찾음');
});
...
async function fetchPendingArticles() {
    try {
        console.log('수정 대기 중 게시글 요청 시작');
        const response = await fetch('/api/v1/admin/articles/updated-pending');
        ...
        console.log('삭제 대기 중 게시글 수신 완료:', articles.length);
        displayArticles(articles);
        updateTotalCount(articles.length, '수정 대기 중');
...
async function fetchApprovedArticles() {
    try {
        console.log('수정 완료됨 게시글 요청 시작');
        const response = await fetch('/api/v1/admin/articles/updated-approved');
        ...
        const articles = await response.json();
        console.log('수정 완료됨 게시글 수신 완료:', articles.length);
        displayArticles(articles);
        updateTotalCount(articles.length, '수정 완료됨');
        setupPagination(articles);
    } catch (error) {
        console.error('게시글 데이터를 가져오는 중 오류 발생:', error);
        handleFetchError();
    ...
    paginatedArticles.forEach(article => {
        ...
        row.innerHTML = `
            <td class="px-6 py-4 whitespace-nowrap">
                <a href="/article/update-detail/${article.id}" class="text-blue-600 hover:underline">
                    ${article.title}
                </a>
            </td>
